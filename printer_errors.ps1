...
printer_error(s) => "8/22"
 ...
 testing "aaaaaaaaaaaaaaaabbbbbbbbbbbbbbbbbbmmmmmmmmmmmmmmmmmmmxyz" "3/56"
 testing "kkkwwwaaaaaaaaaaaaaabbbbbbbbbbbbbbbbbbmmmmmmmmmmmmmmmmmmmxyz" "6/60"
...
function printer-error($s)
{
    $error_count = 0
#    foreach ($i in [char[]]$s) {
#        if ($i -notmatch '[a-m]') {$error_count++}        
#    }    
    [char[]]$s | % {if ($_ -notmatch '[a-m]') {$error_count++}}
